...
def editTheme():
    ...
    validChoice = False
    while not isTheme:
        themeName = inpConf('{0}\nInput the name of the theme you are editing: '.format(themeStr)).capitalize()
    ...
    fileDir = 'themes/'+themeName+'/'
    while True:
        editChoice = inpConf('Editing (C)ard Names or (T)iers (-- to quit): ').lower()
        ...
        if editChoice == 'c'[0]:
            ...
            while True:
                ...
                if validChoice:
                    if cardChoice == 'a':
                        ...
                        nameIndex = cardNamesL.index(nameChoice)
                        if cardChoice == 'e':
            ...
            with open(fileDir+'tnames.txt', 'r') as tNameFile:
                tierNames = tNameFile.readlines()
            with open(fileDir+'tchances.txt', 'r') as tChanceFile:
                tierChances = tChanceFile.readlines()
            tierChoice = inpConf('Editing Tier (N)ames or Tier (C)hances: ')[0].lower()
        elif editChoice == '--':
            break
        else:
            print('Invalid input:', editChoice)
